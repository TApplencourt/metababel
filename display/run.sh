...
ruby ./source_callbacks_generator.rb 3.btx_instances.yaml SOURCE.interval/callbacks.c

# Generate SOURCE.invertal component
$metababel ../main.rb -d 2.interval_definitions.yaml -t SOURCE -p sample -c interval -o SOURCE.interval
...
$metababel ../main.rb -u 2.interval_definitions.yaml -t SINK -p display --params 1.params.yaml -c tally -o SINK.tally
make -C SINK.tally
babeltrace2 --plugin-path=SOURCE.interval:SINK.tally  \
            --component=source.sample.interval \
            --component=sink.display.tally  \
            --params="display_compact=${display_compact},display_human=${display_human}"  
